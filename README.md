...
### 5. 個股深入研究 (Research Report)
> **使用skills：** `stock-deep-research`

Agent 能對個股運用結構化的八階段研究框架，進行全面、多階段的投資盡職調查：

* **全自動執行**：只需提供股票代號（如 `2330` 或 `TSLA`），Agent 即自動完成 14+ 面向的平行數據蒐集，無需額外介入。
* **八階段深度報告**：依序產出公司基本面、產業分析、業務拆解、財務品質、公司治理、市場情緒多空論點、估值與護城河共 7 份專題報告，外加 1 份行政摘要。
* **投資評級與行動建議**：最終彙整為 🟢 Buy / 🟡 Hold / 🔴 Sell 評級，附護城河星等 (1-5⭐)、關鍵風險及核心投資邏輯。
